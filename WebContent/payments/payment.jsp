<%@ page contentType="text/html; charset=UTF-8" %>
<%
  String _r = (String)session.getAttribute("role");
  if (_r == null || !_r.equalsIgnoreCase("admin")) {
    response.sendRedirect(request.getContextPath() + "/authentication/login.jsp");
    return;
  }
%>
<%@ include file="../shared/header.jsp" %>
<%@ include file="../shared/navigation.jsp" %>
<div class="container">
  <h2>Record Payment (Admin)</h2>
  <form method="post" action="paymentProcess.jsp">
    <label>Member ID</label>
    <input type="number" name="member_id" required>
    <label>Amount</label>
    <input type="number" step="0.01" name="amount" required>
    <label>Months</label>
    <input type="number" name="months" value="1" min="1" required>
    <div style="margin-top:1rem"><button type="submit">Submit</button></div>
  </form>
</div>
<%@ include file="../shared/footer.jsp" %>
